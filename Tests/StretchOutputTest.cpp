...
int main()
{
    ...
    std::cout << "Step 1: Filling buffer (500 blocks, Granular)..." << std::endl;
    ...
    // Step 4: Try with freeze=true (plays back buffer without new input)
    std::cout << "\nStep 3: Freeze ON + Stretch..." << std::endl;
    p->freeze = true;
    for (int block = 0; block < 100; ++block)
    {
        clouds::ShortFrame input[kBlockSize], output[kBlockSize];
        std::memset(input, 0, sizeof(input));  // no input
        std::memset(output, 0, sizeof(output));
        processor.Prepare();
        processor.Process(input, output, kBlockSize);
