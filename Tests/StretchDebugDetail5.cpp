...
int main()
{
    ...
    for (int b = 0; b < 500; ++b) {
        ...
        if (b == 0 || b == 99 || b == 499) {
            ...
            std::cout << "  Block " << b << " output max=" << maxO << std::endl;
        }
    }

    // Freeze on and stay in Granular - check if buffer plays back
    std::cout << "\nFreeze ON in Granular mode..." << std::endl;
    p->freeze = true;
    for (int b = 0; b < 20; ++b) {
        clouds::ShortFrame in[BS], out[BS];
        ...
        if (maxO > 100) break;  // Got output!
    }

    // Now switch to Stretch with freeze ON
    std::cout << "\nSwitching to Stretch with freeze ON..." << std::endl;
    ...
    for (int b = 0; b < 20; ++b) {
        ...
        std::cout << "  Stretch freeze block " << b << " max=" << maxO << std::endl;
    }
